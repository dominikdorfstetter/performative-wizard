...
- **Content:** 64 cards, 24 enemy types (incl. **The Critic** & **The Algorithm** bosses),
  3 wizards, 25 outfits, 16 artifacts, 11 events.
...
- **Build:** macOS `.app` (ad-hoc signed) + itch-ready web export, both branded
  (custom icon/boot splash) and versioned. Combat engine covered by **289 headless
...
3. Press **Play** (F5). The console should print
   `[Database] loaded 64 cards, 25 outfits, 24 enemies, 3 wizards, 16 artifacts`.
4. Run the tests headless: `godot --headless scenes/test_combat.tscn` (expect `289 passed, 0 failed`).
...
scripts/
  ...
  tests/             combat_test.gd (289 checks) · balance_sim.gd (headless balance bot)
data/
